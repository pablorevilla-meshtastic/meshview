...
async def migrate_database(db_path: str):
    ...
    async with async_session() as session:
        ...
        await session.execute(text(
            "ALTER TABLE packet ADD COLUMN import_time_us INTEGER"
        ))
        ...
        await session.execute(text(
            "ALTER TABLE packet_seen ADD COLUMN import_time_us INTEGER"
        ))
        await session.commit()
        ...
        await session.execute(text(
            "ALTER TABLE traceroute ADD COLUMN import_time_us INTEGER"
        ))
        await session.commit()
        ...
        result = await session.execute(text(
            "SELECT COUNT(*) FROM packet WHERE import_time_us IS NOT NULL"
        ))
        count = result.scalar()
        ...
        result = await session.execute(text(
            "SELECT COUNT(*) FROM packet_seen WHERE import_time_us IS NOT NULL"
        ))
        count = result.scalar()
        ...
        result = await session.execute(text(
            "SELECT COUNT(*) FROM traceroute WHERE import_time_us IS NOT NULL"
        ))
        count = result.scalar()
